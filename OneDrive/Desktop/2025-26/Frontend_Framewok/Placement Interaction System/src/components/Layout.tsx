...
export function Layout({ 
  ...
  const getSectionDisplayName = (section: string) => {
    const item = items.find(item => item.section === section);
    return item ? item.title : section.charAt(0).toUpperCase() + section.slice(1);
  };
                ...
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-48">
                      ...
                      {navigationHistory.slice().reverse().map((section, index) => {
                        const isCurrentSection = section === currentSection;
                        const reverseIndex = navigationHistory.length - 1 - index;
                        return (
                          <DropdownMenuItem
                            key={`${section}-${reverseIndex}`}
                            onClick={() => {
                              if (!isCurrentSection) {
                                // Navigate to selected section and update history
                                const newHistory = navigationHistory.slice(0, reverseIndex + 1);
                                onNavigate(section);
                              }
                            }}
                            className={isCurrentSection ? 'bg-accent font-medium' : ''}
                          >
                            <div className="flex items-center gap-2">
                              {isCurrentSection && <div className="w-2 h-2 bg-primary rounded-full" />}
                              <span className={isCurrentSection ? 'ml-0' : 'ml-4'}>
              ...
              <div className="ml-auto flex items-center gap-4">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={onLogout} className="text-blue-600 focus:text-blue-600">
                ...
                <BreadcrumbList>
                  {navigationHistory.map((section, index) => (
                    <React.Fragment key={`${section}-${index}`}>
                      ...
                      {index < navigationHistory.length - 1 && <BreadcrumbSeparator />}
                    </React.Fragment>
                  ))}
                </BreadcrumbList>
              </Breadcrumb>
