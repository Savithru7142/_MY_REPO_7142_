...
import { mockCompanyProfiles, CompanyProfile as CompanyProfileType } from '../data/mockData';
...
export function CompanyProfile({ userRole, companyId = '3' }: CompanyProfileProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [companyData, setCompanyData] = useState<CompanyProfileType>(
    mockCompanyProfiles.find(c => c.id === companyId) || mockCompanyProfiles[0]
  );
  ...
  const handleCancel = () => {
    ...
    setCompanyData(mockCompanyProfiles.find(c => c.id === companyId) || mockCompanyProfiles[0]);
    setIsEditing(false);
  };
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <a href={`https://${companyData.website}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                        {companyData.website}
                      </a>
                    </div>
                  </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-semibold">{companyData.openPositions}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-semibold">{companyData.activeRecruiters}</span>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setCompanyData({ ...companyData, culture: newCulture });
                        }}
                      />
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          const newCulture = companyData.culture.filter((_, i) => i !== index);
                          setCompanyData({ ...companyData, culture: newCulture });
                        }}
                      >
                        <X className="h-4 w-4" />
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3 text-muted-foreground">
                <p>
                  We foster an inclusive and collaborative work environment where innovation thrives. 
                  Our team members enjoy flexible work arrangements and continuous learning opportunities.
                </p>
                <p>
                  Regular team-building activities, mentorship programs, and professional development 
                  initiatives ensure that our employees grow both personally and professionally.
                </p>
              </div>
            </CardContent>
          </Card>
